...
class Reward(RewardAPI):
    ...
    # idk how to render rn
    def get_pathches(self):
        hx = self.env.robot.x
        hy = self.env.robot.y

        return [
            patches.Circle((hx, hy), 0.45, color='red', alpha=0.25),
            patches.Circle((hx, hy), 1.2, color='yellow', alpha=0.25),
            patches.Circle((hx, hy), 3.7, color='green', alpha=0.25)
        ]